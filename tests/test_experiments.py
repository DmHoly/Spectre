...
def _launch(client, slug, title="Essai initial", thickness=20):
    ...
    return client.post(f"/api/microprojets/{slug}/experiences", json=body).json()
...
def test_mark_running_then_edit_conclusion(client):
    ...
    assert client.get(f"/api/microprojets/{slug}/experiences/{running_id}").json()["status"] == "running"

    # conclure
    concluded = client.post(
        f"/api/microprojets/{slug}/experiences/{running_id}/conclure",
    ...
    reopened = client.post(f"/api/microprojets/{slug}/experiences/{concluded_id}/statut", json={"status": "running"})
    assert reopened.status_code == 201
    reopened_id = reopened.json()["id"]
    reopened_detail = client.get(f"/api/microprojets/{slug}/experiences/{reopened_id}").json()
    ...
    fixed = client.post(
        f"/api/microprojets/{slug}/experiences/{reopened_id}/conclure",
        json={"status": "concluded", "summary": "Résumé corrigé", "objective_results": [{"objective": "Isolation", "status": "met"}]},
    )
    assert client.get(f"/api/microprojets/{slug}/experiences/{fixed.json()['id']}").json()["conclusion"]["summary"] == "Résumé corrigé"
...
def test_campaign_from_ref_keeps_lineage(client):
    ...
    detail = client.get(f"/api/microprojets/{slug}/experiences/{campaign.json()['id']}").json()
...
def test_delete_experience_removes_the_whole_line_and_refuses_when_forked(client):
    ...
    b2 = client.post(
        f"/api/microprojets/{slug}/experiences/{b}/evoluer",
    ...
    a2 = client.post(
        f"/api/microprojets/{slug}/experiences/{a}/evoluer",
...
def test_evolve_then_timeline_and_diff(client):
    ...
    timeline = client.get(f"/api/microprojets/{slug}/experiences/{evolved_id}/timeline").json()
...
def test_timeline_versions_only_lists_process_changes_full_history_lists_everything(client):
    ...
    evolved = client.post(f"/api/microprojets/{slug}/experiences/{tagged_id}/evoluer", json=evolve_body)
    assert evolved.status_code == 201
    evolved_id = evolved.json()["id"]
...
def test_conclude_experience(client):
    ...
    detail = client.get(f"/api/microprojets/{slug}/experiences/{concluded_id}").json()
...
def test_viewer_cannot_evolve_or_conclude(client):
    ...
    client.post(f"/api/microprojets/{slug}/members", json={"email": "viewer4@example.com", "role": "viewer"})
    ...
    conclude = client.post(f"/api/microprojets/{slug}/experiences/{launched['id']}/conclure", json={"status": "concluded"})
