"""GET /api/atlas: the cross-microproject bird's-eye view (spectre.api.atlas / spectre.core.atlas).
...
def test_atlas_lists_only_the_current_user_own_microprojects(client):
    slug_a = _register_and_microproject(client, "atlas-a@example.com", "Projet A")
    client.post(
        f"/api/microprojets/{slug_a}/experiences",
        json={"substrate": _substrate(), "steps": _steps(), "title": "Etude A", "intent": "Depart"},
    )

    slug_b = _register_and_microproject(client, "atlas-b@example.com", "Projet B")
    client.post(
...
def test_atlas_shows_one_experience_node_per_branch_tip_with_entities_and_objectives(client):
    slug = _register_and_microproject(client, "atlas-tips@example.com")
...
def test_atlas_skips_physical_tracking_entries_with_no_sample_id_or_location(client):
    slug = _register_and_microproject(client, "atlas-empty-entity@example.com")
    launched = client.post(
        f"/api/microprojets/{slug}/experiences",
        json={
            "substrate": _substrate(),
...
def test_atlas_entity_index_survives_a_partially_tracked_campaign(client):
    ...
    """
    slug = _register_and_microproject(client, "atlas-partial-tracking@example.com")
...
def test_atlas_condenses_a_fork_and_merge_into_tip_to_tip_edges(client):
    slug = _register_and_microproject(client, "atlas-merge@example.com")
    root = client.post(
        f"/api/microprojets/{slug}/experiences",
        json={
            "substrate": _substrate(),
