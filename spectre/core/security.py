...
def new_token() -> str:
    """A random, URL-safe token - used for password-reset links and microproject invitations, same
    shape as a session token but generated by a separate call so the two are never confused.
    """
    return secrets.token_urlsafe(32)


def password_reset_expiry() -> str:
    return _sql_datetime(utcnow() + PASSWORD_RESET_LIFETIME)


def invitation_expiry() -> str:
    return _sql_datetime(utcnow() + INVITATION_LIFETIME)
